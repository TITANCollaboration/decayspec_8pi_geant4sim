...
void PrimaryGeneratorGun::ReadFromFile()
{
    ...
    for (G4int i = 0; i < nSets; i++) {
        ...
        inFile >> line >> mode;
        G4cout << "Line: " << line << " Mode: " << mode << G4endl;
    ...
    while (!inFile.eof()) {
        ...
        if( inFile.eof() ) break;
        G4cout << "Particle: " << particle << " Energy: " << energy << " Intensity: " << intensity << G4endl;
    ...
    // temporary fix of duplicate last item  // Jonr : I just corrected how the above code handles new lines so this isn't needed..
